 ...
 * @since 2.0
 */
public class ConnectionPool<T extends Operationable> extends BaseGenericObjectPool<T>
     ...
     *                used by this pool
     */
    public ConnectionPool(final PooledObjectFactory<T> factory) {
    ...
    public void returnObject(final T obj) {
        ...
        final int maxIdleSave = getMaxIdle();
        if (isClosed() || maxIdleSave > -1 && maxIdleSave <= idleObjects.size() && !p.getObject().hasOperations()) {
            try {
                destroy(p);
            } catch (final Exception e) {
                swallowException(e);
        ...
        if (idleObjects.size() > 0) {
            ...
            synchronized (evictionLock) {
                ...
                for (int i = 0, m = getNumTests(); i < m; i++) {
                    ...
                    if (evict && !underTest.getObject().hasOperations()) {
                        destroy(underTest);
